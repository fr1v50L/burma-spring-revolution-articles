    ...
    <React.Fragment>
      ...
      <Layout>
        <div className="max-w-screen-lg md:mx-auto mx-8 my-8">
          <Link href="/bbp-resistance">
            <a className="px-4 py-2 bg-red-600 rounded-lg">
              Back to BBP - Resistance
            </a>
          </Link>
        </div>
...
export async function getStaticProps({ ...ctx }) {
  ...
  const content = await import(`../../posts/bbp-resistance/${postname}.md`);
...
export async function getStaticPaths() {
  ...
  const paths = blogSlugs.map((slug) => `/bbp-resistance/${slug}`);
